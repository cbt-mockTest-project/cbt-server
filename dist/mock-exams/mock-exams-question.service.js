...
let MockExamQuestionService = class MockExamQuestionService {
    ...
    async readMockExamQuestionsByMockExamId(readMockExamQuestionsByMockExamIdInput, user) {
        try {
            const { id, isRandom, bookmarked } = readMockExamQuestionsByMockExamIdInput;
            ...
            if (user) {
                questions = questions.map((question) => {
                    ...
                    const filteredBookmark = question.mockExamQuestionBookmark.filter((bookmark) => user && bookmark.user.id === user.id);
                    return Object.assign(Object.assign({}, question), { state: filteredState, mockExamQuestionBookmark: filteredBookmark });
                });
                if (isRandom) {
